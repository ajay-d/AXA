...
x <- function(x){
  ...
  for(d in drivers) {
    ...
    rf <- randomForest(as.factor(target) ~ ., data=model.data, type='classification', 
                      n.trees=10000, importance=TRUE)
    print(rf)
    importance(rf)
    
    
    p <- predict(rf, curDriver, type="prob")
    p <- p[,'1']
...
file <- paste0("axa-random7-10k-seed=1111", ".csv.gz")
