...
x <- function(x){
  ...
  for(d in drivers) {
    ...
    nonDrivers <- driver.summary[sample(nrow(nonDrivers), 1000)] %.%
      mutate(target = 0)
    ...
    g <- gbm(target ~ ., data=model.data, dist='bernoulli', n.trees=25000,
             interaction.depth=4, n.minobsinnode=5, shrinkage=.001, train.fraction=.90,
...
file <- paste0("axa-gbm6-25k-depth=4-train=.90-shrink=.001-seed=3333", ".csv.gz")
