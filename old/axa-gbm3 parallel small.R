...
x <- function(x){
  ...
  for(d in drivers) {
    ...
    model.data <- rbindlist(list(curDriver %>% sample_frac(.75), nonDrivers), use.names=TRUE) %>%
      select(-driver, -trip)
      
    model.data <- sapply(model.data, as.numeric) %>%
      as.data.frame
    
    g <- gbm(target ~ ., data=model.data, dist='bernoulli', n.trees=5000,
...
file <- paste0("axa-gbm3-5k-depth=2-train=.75-shrink=.001-seed=1111", ".csv.gz")
