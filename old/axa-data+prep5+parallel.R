...
x <- function(x){
  ...
  for(driver in drivers) {
    ...
    for(i in 1:200) {
      ...
      theta.diff.smooth.q <- quantile(theta.diff.smooth$fitted, seq(0.05, 1, by = 0.05), na.rm=TRUE)
      ...
      currentData <- c(driver=driver,trip=i, duration=nrow(trip), 
                 ...
                 total.turn = sum(trip.turns$x.turn, na.rm=T) + sum(trip.turns$y.turn, na.rm=T),
                 ...
                 dist.q, theta.q, theta.diff.q, theta.sample.diff.q, theta.diff.smooth.q,
...
save(driver.summary, file='drivers5.RData')
